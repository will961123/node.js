 ...
 * 当接收到数据时触发。
 * end
 * 当socket 另一端发送 FIN 包时，触发该事件。
 * timeout
 * 当socket 空闲超时时触发，仅是表明 socket 已经空闲。用户必须手动关闭连接
 * drain
 * 当写缓存为空得时候触发。可用来控制上传。
 * error
 * 错误发生时触发。
 * close
 * 当socket 完全关闭时触发。参数 had_error 是布尔值，它表示是否因为传输错误导致 socket 关闭。
 *
 * 属性
 * 	socket.bufferSize
 * 该属性显示了要写入缓冲区的字节数。
 ...
 * 在socket 上发送数据。第二个参数指定了字符串的编码，默认是 UTF8 编码。
 ...
 * 设置为 true 将会在调用 socket.write() 时立即发送数据。noDelay 默认值为 true。
 * socket.setKeepAlive([enable][, initialDelay])
 * 禁用/启用长连接功能，并在发送第一个在闲置 socket 上的长连接 probe 之前，可选地设定初始延时。默认为 false。
 * 设定 initialDelay （毫秒），来设定收到的最后一个数据包和第一个长连接probe之间的延时。
...
let server = net.createServer(connection => {
  console.log("客户端连接");
  connection.on("end", () => {
    console.log("客户端关闭连接1");
  });
  connection.write("客户端关闭连接2！\r\n");
  connection.pipe(connection);
});
server.listen(8080, () => console.log("服务器正在监听8080"));
