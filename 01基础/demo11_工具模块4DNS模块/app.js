 ...
 * 将域名（比如 'runoob.com'）解析为第一条找到的记录 A （IPV4）或 AAAA(IPV6)。参数 options可以是一个对象或整数。
 * 如果没有提供 options，IP v4 和 v6 地址都可以。如果 options 是整数，则必须是 4 或 6。
 ...
 * 和dns.resolve4() 类似， 仅能查询 IPv6( AAAA 查询）
 * dns.resolveMx(hostname, callback)
 * 和 dns.resolve() 类似, 仅能查询邮件交换(MX 记录)。
 * dns.resolveTxt(hostname, callback)
 * 和 dns.resolve() 类似, 仅能进行文本查询 (TXT 记录）。 addresses 是 2-d 文本记录数组。(比如，[ ['v=spf1 ip4:0.0.0.0 ', '~all' ] ]）。
 ...
 * 和 dns.resolve() 类似, 仅能进行域名服务器记录查询(NS 记录）。 addresses 是域名服务器记录数组（hostname 可以使用）
 * (比如, ['ns1.example.com', 'ns2.example.com']）。
...
dns.lookup("www.github.com", (err, address, family) => {
  ...
  dns.reverse(address, (err, hostname) => {
    ...
    console.log("反向解析", address, "为:", JSON.stringify(hostname));
  });
});
